...
- Meal-note nutrient estimates are generated by OpenAI when `OPENAI_API_KEY` is set, with a local fallback when it is not
...
`OPENAI_API_KEY` is optional. If set, the parent dashboard will use OpenAI both to estimate meal nutrients from the nanny's text notes and to rewrite the computed signals into a tighter summary. Without it, the dashboard uses built-in rule-based summaries and heuristic nutrient estimation.
